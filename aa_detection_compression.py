...
#####New Imports from Me#########
from aa_compressor import compress_features
from aa_utils import set_seed
########################################

# Globale Seed-Setzung direkt zu Beginn
set_seed(42)
...
class Detection:
    ...
    def __init__(self):
        ...
        # Setze Modell und Latent-Dimension
        self.compression_method="pca" # "autoencoder","pca", "none"
        self.latent_dim= 0.9
        #self.latent_dim= int(0.003 * features.shape[1])
    ...
    def _strategy_runner_process(self, args):
        ...
        """
        ##########
        from aa_utils import set_seed  # Importiere die set_seed Funktion innerhalb des Prozesses
        set_seed(42)  # Seed innerhalb des Prozesses setzen
        ##########
...
#####################################################################################################################################
    ...
    def generate_features(self, d): # compression_method, latent_dim
        """
        This method generates and compresses features for each column.
        """
        ...
        for j in range(d.dataframe.shape[1]):
            ...
            # Compress the features using the specified method
            compressed_features = compress_features(
                feature_vectors,
                method=self.compression_method,
                ratio=self.latent_dim  #
            )
            
            if self.VERBOSE:
                print("{} Features are generated for column {}.".format(compressed_features.shape[1], j))
            
            columns_features_list.append(compressed_features)
...
#####################################################################################################################################
    ...
    def sample_tuple(self, d):
        ...
        numpy.random.seed(42) # new random
        ...
        for j in range(d.dataframe.shape[1]):
            ...
            if sum(y_train) == len(y_train):
                ...
                if self.CLASSIFICATION_MODEL == "ABC":
                    classification_model = sklearn.ensemble.AdaBoostClassifier(n_estimators=100,random_state=42) # new random
                ...
                if self.CLASSIFICATION_MODEL == "GBC":
                    classification_model = sklearn.ensemble.GradientBoostingClassifier(n_estimators=100,random_state=42) # new random
    ...
    def run(self, dd):
        ...
        if self.VERBOSE:
            print("------------------------------------------------------------------------\n"
                  ...
                  "------------------------------------------------------------------------")
        self.generate_features(d) # self.compression_method, self.latent_dim
        if self.VERBOSE:
            print("------------------------------------------------------------------------\n"
...
if __name__ == "__main__":
    ...
    # Globale Seed-Setzung zum Programmstart
    set_seed(42)
